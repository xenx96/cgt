...
//그러므로 set 함수를 권장..
      ...
      .then(response => {
        ...
        setEncryptedSummoner(response.data.id)
...
// encryptedSummoner값이 변하면 함수 실행
      ...
      axios.get(`https://kr.api.riotgames.com/lol/league/v4/entries/by-summoner/${encryptedSummoner}?api_key=${api_key}`)
      .then(response => {
        setDatas(response.data);
        console.log(response.data);
        setLoading(false);
        //데이터를 받아온 후 값을 리턴해야 하므로 로딩 변수를 설정하여 값을 받은 후 리턴값을 받아올수 있게 설정
      })},[encryptedSummoner])
