...
function Contents({ match }) {
  ...
  const fetchSumData = async (encryptedsummoner) => {
    ...
    try {
      res = await axios.get(
        `https://kr.api.riotgames.com/lol/league/v4/entries/by-summoner/${encryptedsummoner.id}?api_key=${api_key}`
      );
  ...
  const fetchLeagueTeam = async (Datas) => {
    ...
    try {
      res = await axios.get(
        `https://kr.api.riotgames.com/lol/league/v4/leagues/${Datas[0].leagueId}?api_key=${api_key}`
      );
  ...
  return (
    <content>
      <nav className="menu">
        <SummonerCard leaguedata={datas} sumdata={encryptedSummoner} />
        {/* (소환사 카드부분 컴포넌트) 필요한데이터=> 리그데이터, 소환사 이름*/}
        <dl className="menu_buttons">
          <dd>
            <a href="#" className="menu_btn all_btn">
              종합
            </a>
          </dd>
          <dd>
            <a href="#" className="menu_btn champ_btn">
              챔피언
            </a>
          </dd>
          <dd>
            <a href="#" className="menu_btn ingame_btn">
              🔍인게임 정보
            </a>{" "}
            {/*위의 부분은 데이터가 필요없음, 컴포넌트화 하면 보기편하지만 테스트용이므로 패스*/}
          </dd>
        </dl>
      </nav>
      <div className="main_contents">
        <SeasonCard leaguedata={datas} leagueteam={leagueTeam} />{" "}
        {/*시즌 솔로랭크, 자유랭크 티어 정보 카드, 필요한데이터 => 리그데이터, 리그이름*/}
        {/*
        ...
        <div className="all_game_infocard">
            ...
            <div className="detail_info">
              <div className="losewin_rate">
                <div className="first_box win_graph">
                  <span className="win_ratio_title">
                    <span>0</span>전<span>0</span>승<span>0</span>패
                  </span>
                ...
                <div className="first_box kda_info">
                  <span className="kda">
                    <span>0</span>/<span className="f_red">0</span>/
                    <span>0</span>
                  </span>
                  <span className="kda_ratio">
                    <span>0</span>:<span>0</span>{" "}
                    <span className="f_red">(0%)</span>
                  </span>
                </div>
              </div>
          ...
          <div className="game_card">
            ...
            <div className="detail_kda">
              <span className="kda_stat fnt_15">
                0/<span className="f_red">0</span>/0
              </span>
